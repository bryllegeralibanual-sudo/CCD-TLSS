              ...
              {!isResolved && (
                <div>
                  <button
                    ...
                    className="mb-3 flex items-center gap-2 rounded-lg bg-emerald-100 px-3 py-2 text-xs font-black text-emerald-800 hover:bg-emerald-200"
                  >
                    <RefreshCw size={12} /> {showSuggestions[conflict.id] ? 'Hide' : 'Show'} Alternative Times ({alternatives.length})
                  </button>

                  {showSuggestions[conflict.id] && (
                    <div className="grid gap-2 rounded-lg bg-white p-3 md:grid-cols-2">
                      {alternatives.slice(0, 6).map((alt, i) => (
                        <button
                          key={i}
                          onClick={() => setSelectedAlternatives(prev => ({ ...prev, [conflict.id]: alt }))}
                          className="rounded-lg border-2 border-emerald-200 bg-emerald-50 p-2 text-left text-xs hover:bg-emerald-100"
                        >
